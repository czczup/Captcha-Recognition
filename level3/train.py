...
def network():
    ...
    prediction = tf.nn.softmax(output) # 31
    label = tf.one_hot(indices=tf.cast(Y, tf.int32), depth=31, name='y_onehot')
...
def load_dataset():
    ...
    with tf.name_scope('input_train'):
        ...
        image_batch_train, label_batch_train = tf.train.shuffle_batch(
            [image_train, label_train], batch_size=256, capacity=10240, min_after_dequeue=5120
        )
    # Load validation set.
    with tf.name_scope('input_valid'):
        ...
        image_batch_valid, label_batch_valid = tf.train.shuffle_batch(
            [image_valid, label_valid], batch_size=256, capacity=5120, min_after_dequeue=2560
        )
