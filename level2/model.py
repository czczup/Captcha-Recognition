...
with tf.name_scope('input'):
    X = tf.placeholder(tf.float32, [None, 44,60,1], name='X')
...
def captcha_cnn(w_alpha=0.01, b_alpha=0.1):
    ...
    with tf.name_scope('CONV4'):
        ...
        drop_c4 = tf.nn.dropout(pool4, keep_prob,name='drop_c4')
    # 3*4
    with tf.name_scope('FC1'):
        dense = tf.reshape(drop_c4, [-1, 3*4*512],name='dense')
        w_fc1 = tf.Variable(w_alpha * tf.random_normal([3*4*512, 1024]),name='w_fc1')
    ...
    with tf.name_scope('FC2'):
        w_fc2 = tf.Variable(w_alpha * tf.random_normal([1024, 32]),name='w_fc2')
